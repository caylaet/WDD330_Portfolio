...
export default class Auth {
    constructor(errorHandler) {
        this.jwtToken = '';
        this.user = {};
        // Used to claim any errors from the login process
    ...
    async login(callback) {
        ...
        try {
            ...
            // clear the password
            password.value = '';
            // Clear any errors from the errorHandler
    ...
    async getCurrentUser(email) {
        try {
            ...
            const user = await makeRequest('users?email='+email, 'GET', null, this.jwtToken);
            console.log(user);
            // Return the first item (username) in the response object
            return user[0];
        } catch (error) {
            // if there were any errors display them
            this.errors.handleError(error);
